...
namespace Server.FeatInfo
{
	...
	public abstract class BaseFeat : object
	{
		...
		public virtual Civilization[] AllowedNations{ get{ return new Civilization[]{ }; } }
		...
		public static string GetRacialRequirements( BaseFeat skill )
		{
			...
			if( skill.IsRacialFeat )
			{
				for( int i = 0; i < skill.AllowedNations.Length; i++ )
				{
					...
					if( skill.AllowedNations[i] != Civilizations.Aludia && skill.AllowedNations[i] != Civilizations.Durah )
		...
		public bool MeetsTheRacialRequirement( PlayerMobile m )
		{
			...
			{
				if( AllowedNations[i] == m.osu_char_info.civilization )
					return true;
			}
		...
		public virtual bool MeetsOurRequirements( PlayerMobile m )
		{
			...
			for( int i = 0; i < Requires.Length; i++ )
			{
				...
				if( m.osu_char_info.Feats.GetFeatLevel(skill) > 2 )
					return true;
			}
		...
		{
			return (m.osu_char_info.Feats.GetFeatLevel(ListName) > 2);
		}
  ...
  //              OnLevelLowered( m );
		...
		public bool CanRemoveThisFeat( PlayerMobile m, bool freeRemoval )
		{
			...
			{
				if( m.osu_char_info.Feats.GetFeatLevel(Allows[i]) > 0 && !m.osu_char_info.Feats.FeatDictionary[Allows[i]].CanKeepThisFeatWithout(m, this) )
				{
					m.SendMessage( "If you wish to remove this skill, first you will have to remove " + m.osu_char_info.Feats.FeatDictionary[Allows[i]].Name + " or " +
			...
			if( !freeRemoval )
			{
				if( m.Backpack != null && m.Backpack.ConsumeTotal(typeof(Gold), (Level * BaseCost)) )
				   return true;
		...
		public bool CanKeepThisFeatWithout( PlayerMobile m, BaseFeat toRemove )
		{
			...
			for( int i = 0; i < Requires.Length; i++ )
			{
				if( Feats.Catalogue[Requires[i]].ListName != toRemove.ListName && m.osu_char_info.Feats.GetFeatLevel(Requires[i]) > 2 )
					return true;
			}
		...
		public virtual void AttemptPurchase( PlayerMobile m, int level, bool freeRemoval )
		{
			...
			//check here if change is CP, PP, HP, MP
			else if( m.osu_char_info.current_combat_points < CostToRaise )
